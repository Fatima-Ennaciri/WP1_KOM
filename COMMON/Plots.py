...
def prepareColorBar(PlotConf, ax, Values):
    ...
    if "" in PlotConf:
        ...
        label=PlotConf["ColorBarLabel"],ticks=PlotConf["ColorBarTicks"])
...
def generateLinesPlot(PlotConf):
    ...
    if "Legend" in PlotConf:
            ax.legend(PlotConf["yData"].keys(), loc ='best')
...
def generateRimsMapPlot(PlotConf):
    LineWidth = PlotConf["LineWidth"]
    ...
    ax.scatter(PlotConf["xData"],PlotConf["yData"],marker = PlotConf["Marker"],c = PlotConf["Color"],LineWidth = LineWidth)
    x = PlotConf["xData"]
    y = PlotConf["yData"]
    Text = PlotConf["Text"]
    for i in range(1,40):
        i -=1
        ax.text(x[i],y[i],Text[i],fontsize = 6)
...
def generateBarPlot(PlotConf):
    ...
    for Label in PlotConf["yData"].keys():
            ax.bar(PlotConf["xData"][Label],PlotConf["yData"][Label],Color = PlotConf["Color"])
            ax.axhline(1, color ='green', lw = 1)
             
    plt.legend(PlotConf["Legend"].values())

    saveFigure(fig, PlotConf["Path"])

def generateBar1Plot(PlotConf):
...
def generateBar2Plot(PlotConf):
    ...
    for Label in PlotConf["yData"].keys():
            ax.bar(PlotConf["xData"][Label],PlotConf["yData"][Label],Color = PlotConf["Color"][Label])
...
def generatePlot(PlotConf):
    if(PlotConf["Type"] == "Lines"):
        generateLinesPlot(PlotConf)
    elif(PlotConf["Type"] == "Bar"):
        generateBarPlot(PlotConf)    
    elif(PlotConf["Type"] == "Bar1"):
        generateBar1Plot(PlotConf)
    elif(PlotConf["Type"] == "Bar2"):
        generateBar2Plot(PlotConf)
    elif(PlotConf["Type"] == "Map"):
        generateRimsMapPlot(PlotConf)
    elif (PlotConf["Type"] == "point"):
